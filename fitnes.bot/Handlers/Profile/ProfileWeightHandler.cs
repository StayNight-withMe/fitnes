...
public class ProfileWeightHandler : IBotHandler
{
    ...
    public ProfileWeightHandler(ITelegramBotClient botClient, ISessionRepository sessionRepository, IRequestContext context, IMediator mediator, IOptionsMonitor<SessionOptions> sessionOptions)
    {
    ...
    {
        return update.CallbackQuery?.Data is string data && data.Equals(CallbackPrefixConstants.ProfileWeightEdit);
    }

    public async Task HandleAsync(Update update, CancellationToken ct)
    {
        var chatId = update.CallbackQuery!.From.Id;
        var result = await _mediator.Send(new StartWeightEditMessage(chatId), ct);
